...
{
	int items[] = { 4, 8, 12 };
	const ds::my_dynamic_list list(items, 3);
	ASSERT_EQ(list[0], 4);
	ASSERT_EQ(list[1], 8);
	ASSERT_EQ(list[2], 12);
}
...
{
	const ds::my_dynamic_list list(2);
...
TEST(MyDynamicList, Constructor_WithCapacity0_ShouldThrow)
{
	EXPECT_THROW
	({
		const ds::my_dynamic_list list(0);
	}, std::exception);
}
...
{
	int items[] = { 4, 8, 12 };
	ds::my_dynamic_list list(items, 3);
	const ds::my_dynamic_list clone = list;
...
{
	int items[] = { 2, 4, 6, 8 };
	ds::my_dynamic_list list(items, 4);
...
{
	int items[] = { 1, 2, 3, 4, 5 };
	ds::my_dynamic_list list(items, 5);
